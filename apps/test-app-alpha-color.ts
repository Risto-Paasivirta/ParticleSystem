...
class ExampleTest {
    ...
    init(): void {
        ...
        for (let i = 0; i < maxSprites; i++) {
            ...
            const sprite = new PIXI.Sprite(PIXI.utils.TextureCache["smoke_10.png"]);
    ...
    update(dt: number) {
        ...
        for (let i = 0; i < maxSprites; i++) {
            if (i >= this.particleSystem.particles.length) {
                ...
                this.cachedSprites[i].x = particle.position.x + 500;
                this.cachedSprites[i].y = particle.position.y + 300;
                this.cachedSprites[i].alpha -= Math.random() * 0.01;
            }
        }
    }
}
